...
export class QualifiersComponent extends AppComponent implements OnInit {
    ...
    async ngOnInit(): Promise<void> {
        let pools = await this.http
            .get(`api/tournament/${this.tournament.id}/map-pools`)
            .toPromise();
        ...
        this.getQuals().subscribe((res) => {
            ...
            for (const user of this.qualsScores) {
                ...
                for (const score of user.scores) {
                    if (
                        this.qualsPool.songs.find(
                            (x) => x.hash == score.songHash
                        )?.numNotes != 0
                    ) {
                        score.percentage =
                            score.score /
            ...
            for (const leaderboard of Object.keys(this.leaderboards)) {
                this.leaderboards[leaderboard].sort(
                    (a, b) => b.score - a.score
                );
            }
            for (const user of this.qualsScores) {
                for (const score of user.scores) {
                    score.position = this.leaderboards[
                        score.songHash
                    ].findIndex((x) => x.discordId == user.discordId);
                }
                user.scores.sort((a, b) =>
                    a.songHash > b.songHash
                        ? 1
                        : a.songHash < b.songHash
                        ? -1
                        : 0
            ...
            if (this.qualsScores.length == 1) {
                let sumA = this.sumProperty(
                    this.qualsScores[0].scores,
                    'score'
                );
                let sumAPer = this.sumProperty(
                    this.qualsScores[0].scores,
                    'percentage'
                );
                this.qualsScores[0].avgPercentage = isNaN(
                    (sumAPer / this.qualsPool.songs.length) * 100
                )
                    ? 0
                    : ((sumAPer / this.qualsPool.songs.length) * 100).toFixed(
                          2
                      );
            ...
            this.qualsScores.sort((a, b) => {
                ...
                b.avgPercentage = isNaN(
                    (sumBPer / this.qualsPool.songs.length) * 100
                )
                    ? 0
                    : ((sumBPer / this.qualsPool.songs.length) * 100).toFixed(
                          2
                      );
                a.scoreSum = sumA;
                b.scoreSum = sumB;
            ...
            this.qualsScores.splice(
                this.tournament.tournament_settings.quals_cutoff,
                0,
                {
                    cutoff: true,
                }
    ...
    taSync() {
        const dialog = this.dialog.open(ConfirmDialogComponent, {
            ...
            data: {
                ...
                confirmText: 'Sync',
                message:
                    'Syncing scores with TA will remove all current scores and ensure that they match the TA leaderboards, this will bypass elements such as qualifier attempt limits or remove old scores that are no longer saved within TA.',
        ...
        dialog.afterClosed().subscribe(async (data) => {
            if (data) {
                try {
                    await this.http
                        .put(
                            `/api/tournament/${this.tournament.id}/qualifiers/sync`,
                            {}
                        )
                        .toPromise();
                    this.notif.showInfo(
                        '',
                        'Scores are being synced. Please refresh in a minute'
                    );
                } catch (error) {
    ...
    public getQuals(): Observable<any> {
        return this.http.get(
            `/api/tournament/${this.tournament.id}/qualifiers`
        );
    }
