...
export class MapPoolComponent extends AppComponent implements OnInit {
    ...
    async ngOnInit(): Promise<void> {
        let poolStaff = this.staff.filter((x) =>
            x.roles.some((x) => x.id == 2)
        );
        await this.userS.curUser();
        if (
            this.user != null &&
            (this.user.roles.find((x) => x.roleId === 1) ||
                this.user.discordId == this.tournament.owner ||
                poolStaff.find((x) => x.discordId == this.user.discordId))
        ) {
            this.isAuthorised = true;
            this.columnsToDisplay.push('delete');
        }
        this.getPools().subscribe((data) => {
            this.loading = false;
            this.mapPools = data;
    ...
    public logIn(): Observable<IUser.User[]> {
        return this.http.get<IUser.User[]>('/api/user');
    }
    ...
    delete(songId) {
        const dialog = this.dialog.open(ConfirmDialogComponent, {
            ...
            data: {
                ...
                confirmText: 'Delete',
                message:
                    'Are you sure you want to delete this song from this pool?',
                title: 'Delete Song?',
            },
        });
        ...
        dialog.afterClosed().subscribe(async (data) => {
            if (data) {
                ...
                try {
                    await this.http
                        .post(
                            `api/tournament/${this.tournament.id}/deleteSong`,
                            info
                        )
                        .toPromise();
                    this.updatePools();
                    ...
                    this.notif.showError(
                        '',
                        'Error deleting song from map pool'
                    );
                }
            }
        });
    ...
    downloadPool() {
        let curSongs = this.mapPools[this.curPoolId].songs.map((e) => {
            return { hash: e.hash };
        });
        let playlist = {
        ...
        downloadAnchorNode.setAttribute(
            'download',
            `${this.tournament.name}_` +
                this.mapPools[this.curPoolId].poolName +
                '.json'
        );
    ...
    public getPools(): Observable<any[]> {
        return this.http.get<any[]>(
            `api/tournament/${this.tournament.id}/map-pools`
        );
    }

    async updatePools() {
        let data = await this.http
            .get<any[]>(`api/tournament/${this.tournament.id}/map-pools`)
            .toPromise();
        this.mapPools = data;
        this.poolsLen = Object.keys(this.mapPools).length;
        this.poolValues = Object.values(this.mapPools);
        ...
        dialog.afterClosed().subscribe(async (data) => {
            if (data) {
                try {
                    await this.http
                        .delete(
                            `api/tournament/${this.tournament.id}/map-pools/${this.curPoolId}`
                        )
                        .toPromise();
                    this.updatePools();
                    this.curPoolId = Object.keys(this.mapPools)[0];
    ...
    ngOnInit() {
        if (this.data.edit) {
            ...
            this.newPoolForm = this.fb.group({
                ...
                poolName: [this.data.mapPool.poolName, [Validators.required]],
                image: [this.data.mapPool.image, [Validators.required]],
    ...
    async onSubmit() {
        ...
        if (this.data.edit) {
            ...
            try {
                await this.http
                    .put(
                        `/api/tournament/${this.data.tournament.tournamentId}/map-pools`,
                        this.newPoolForm.value
                    )
                    .toPromise();
            ...
            try {
                await this.http
                    .post(
                        `/api/tournament/${this.data.tournament.tournamentId}/addPool`,
                        this.newPoolForm.value
                    )
                    .toPromise();
                this.notif.showSuccess('', 'Successfully updated map pool');
...
export class addSongDialog implements OnInit {
    ...
    ngOnInit() {
        this.newSongForm = this.fb.group({
            ...
            poolIds: [[], [Validators.minLength(1)]],
            ssLink: ['', [Validators.required]],
            diff: '',
        });
    }
    ...
    onCheckChange(event) {
        ...
        if (event.checked && !this.newSongForm.value.poolIds.includes(poolId)) {
            ...
            this.newSongForm.value.poolIds.splice(
                this.newSongForm.value.poolIds.findIndex((e) => e === poolId),
                1
            );
    ...
    async onSubmit() {
        ...
        try {
            await this.http
                .post(
                    `/api/tournament/${this.data.tournament.id}/addSong${
                        this.beatsaver ? 'ByKey' : ''
                    }`,
                    this.newSongForm.value
                )
                .toPromise();
