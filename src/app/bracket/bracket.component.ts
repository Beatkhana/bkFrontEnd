...
export class BracketComponent extends AppComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.ws.next({ setTournament: this.tournament.id });
    }
    ...
    async initSettings() {
        // const matchesData: any = await this.http.get(`/api/tournament/${this.tournament.id}/bracketTest`).toPromise();
        const matchesData: any = await this.http
            .get(`/api/tournament/${this.tournament.id}/bracket`)
            .toPromise();
        ...
        if (matchesData.length > 0)
            BracketComponent.generateMatches(
                this.bracketData,
        ...
        for (let i = 0; i < matchElements.length; i++) {
            ...
            element.addEventListener('click', () =>
                this.updateMatch(element.getAttribute('data-matchid'))
            );
        }
        this.loading = false;
    }
    ...
    updateMatch(id) {
        const dialog = this.dialog.open(updateMatchDialog, {
            ...
            data: {
                ...this.bracketData.find((x) => x.id == id),
                isAuth: this.isAuth,
            },
        });
    ...
    updateDrawnMatch(data: bracketMatch) {
        ...
        matchElem.querySelector('.pName.p1').innerHTML =
            data.p1.name || data.p1.id;
        matchElem.querySelector('.pName.p2').innerHTML =
            data.p2.name || data.p2.id;
        BracketComponent.generateLabels(this.bracketData);
        if (data.status == 'in_progress') {
            (<SVGPathElement>(
                matchElem.querySelector('.in_progress')
        ...
        if (
            ...
            matchElem.querySelector('.pScore.p1').innerHTML =
                data.p1.score.toString();
        if (
            data.p2.score != 0 ||
            data.status == 'in_progress' ||
            data.status == 'complete'
        )
            matchElem.querySelector('.pScore.p2').innerHTML =
                data.p2.score.toString();

        matchElem
            .querySelector('.img.p1')
            ?.setAttribute(
                'href',
                data.p1.avatar
                    ? `href="https://cdn.discordapp.com/avatars/${data.p1.id}${data.p1.avatar}"`
                    : ''
            );
        matchElem
            .querySelector('.img.p2')
            ?.setAttribute(
                'href',
                data.p2.avatar
                    ? `href="https://cdn.discordapp.com/avatars/${data.p2.id}${data.p2.avatar}"`
                    : ''
            );
        matchElem.classList.remove('hidden');
        let lineElem = document.querySelectorAll(
            `[data-round="${data.round}"][data-match="${data.matchNum}"]`
        );
    ...
    static generateMatches(bracketMatches, intervalIteration) {
        ...
        const height =
            losersMatches.length > 0
                ? winnersMatches[0].length * 60 +
                  losersMatches[0].length * 60 +
                  200
                : winnersMatches[0].length * 60 + 200;
        const width =
            losersMatches.length > 0
                ? losersMatches.length * 200 + 150
                : winnersMatches.length * 200 + 150;
        ...
        for (let i = 0; i < winnersMatches.length; i++) {
            const round = matches[i];
            const gRound = document.createElementNS(
                'http://www.w3.org/2000/svg',
                'g'
            );
            gRound.setAttribute(
                'transform',
                'translate(' + (i * 220 - 100) + ', 30)'
            );
            ...
            for (let j = 0; j < round.length; j++) {
                const match = round[j];
                let svgMatch = BracketComponent.createSvgMatch(
                    j,
                    match,
                    bracketMatches,
                    round,
        ...
        for (let i = 0; i < losersMatches.length; i++) {
            ...
            gRound.setAttribute(
                'transform',
                'translate(' +
                    (loserMatchCounter * 220 - 100) +
                    ', ' +
                    (winnersMatches[0].length * 60 + 150) +
                    ')'
            );
        ...
        const gLines = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'g'
        );
        gLines.classList.add('lines');
        const gHeaders = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'g'
        );
        ...
        for (let i = 0; i < winnersMatches.length - 1; i++) {
            ...
            for (let j = 0; j < round.length; j++) {
                ...
                let matchLine = document.createElementNS(
                    'http://www.w3.org/2000/svg',
                    'path'
                );
                matchLine.setAttribute(
                    'd',
                    `M${curX} ${curY} l${xDist / 4} 0 l${xDist / 2} ${yDist} l${
                        xDist / 4
                    } 0`
                );
                ...
                if (
                    match['bye'] ||
                    (winnersMatches[i + 1][Math.floor(j / 2)] != undefined &&
                        winnersMatches[i + 1][Math.floor(j / 2)]['bye'])
                ) {
            ...
            const roundLabel = document.createElementNS(
                'http://www.w3.org/2000/svg',
                'g'
            );
            roundLabel.setAttribute(
                'transform',
                'translate(' + (i * 220 - 100) + ', 0)'
            );
        ...
        for (
            let loserMatchCounter = 0;
            loserMatchCounter < losersMatches.length - 1;
            loserMatchCounter++
        ) {
            ...
            for (let j = 0; j < round.length; j++) {
                const match: bracketMatch = round[j];
                var multi =
                    i != 0 ? 60 * 2 ** Math.floor(loserMatchCounter / 2) : 60;
                // var startPos = 0.5 * (2 ** i) * 60;
                var startPos =
                    round.length != 1
                        ? 0.5 * 2 ** Math.floor(loserMatchCounter / 2) * 60 +
                          (winnersMatches[0].length * 60 + 150)
                        : 0.5 *
                              2 ** (Math.floor(singleMatchRound / 2) + 1) *
                              60 +
                          (winnersMatches[0].length * 60 + 150);
                ...
                var multi2 =
                    i + 1 != 0 ? 60 * 2 ** (Math.floor(i / 2) + 1) : 60;
                var startPos2 =
                    round.length != 1
                        ? 0.5 * 2 ** (Math.floor(i / 2) + 1) * 60 +
                          (winnersMatches[0].length * 60 + 150)
                        : 0.5 *
                              2 ** (Math.floor(singleMatchRound / 2) + 1) *
                              60 +
                ...
                if (
                    match['bye'] ||
                    (losersMatches[loserMatchCounter + 1][Math.floor(j / 2)] !=
                        undefined &&
                        losersMatches[loserMatchCounter + 1][Math.floor(j / 2)][
                            'bye'
                        ])
            ...
            roundLabel.setAttribute(
                ...
                'translate(' +
                    (i * 220 - 100) +
                    ', ' +
                    (winnersMatches[0].length * 60 + 120) +
                    ')'
            );
            const labelRect = document.createElementNS(
                'http://www.w3.org/2000/svg',
        ...
        document.querySelectorAll('.pName').forEach((elem) => {
            elem.addEventListener('mouseover', function () {
                document.querySelectorAll(`.pName`).forEach((name) => {
                    if (name.innerHTML == elem.innerHTML)
    ...
    static createSvgMatch(
        i,
        bracketMatch: bracketMatch,
        bracketData: bracketMatch[],
        roundElem,
        singleMatchRound: number,
        intervalIteration = 0,
        losers = false
    ) {
        ...
        var startPos =
            roundElem.length != 1
                ? 0.5 * 2 ** round * 60
                : 0.5 * 2 ** singleMatchRound * 60;
        if (losers) {
            var multi = round != 0 ? 60 * 2 ** Math.floor(round / 2) : 60;
            var startPos =
                roundElem.length != 1
                    ? 0.5 * 2 ** Math.floor(round / 2) * 60
                    : 0.5 * 2 ** (Math.floor(singleMatchRound / 2) + 1) * 60;
        }
        group.setAttribute(
            'transform',
            'translate(0, ' + (multi * i + startPos) + ') scale(0.8)'
        );
        ...
        const clip1 = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'clipPath'
        );
        ...
        const clip2 = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'clipPath'
        );
        ...
        const nameClip1 = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'clipPath'
        );
        ...
        const nameClip2 = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'clipPath'
        );
        ...
        group.innerHTML += `
            ...
            <path class="matchPath in_progress" ${
                bracketMatch.status != 'in_progress'
                    ? 'style="display:none;"'
                    : ''
            } id="livePath-${i}" d="m 0 12.25 l 0 -12.5 l 15 -15 h 200 l 15 15 l 0 12.5 M 0 12.25 l 0 12.5 l 15 15 h 200 l 15 -15 l 0 -12.5" />
            <path class="matchPath ${noAnim} matchSplit" d="m 0 12.25 l 230 0" />`;

        let p1Name: string = bracketMatch.p1.name;
        if (bracketMatch.p1.name == null && bracketMatch.p1.id != null)
            p1Name = bracketMatch.p1.id;

        group.innerHTML += `
            <image x="17" y="-10"
            ${
                bracketMatch.p1.avatar
                    ? `href="https://cdn.discordapp.com/avatars/${bracketMatch.p1.id}${bracketMatch.p1.avatar}"`
                    : ''
            }            
            class="img p1" height="16" width="16" clip-path="url(#clipPath-${
                bracketMatch.id
            }-1)" />
            <text x="37" y="5" width="147" height="12" class="pName p1" clip-path="url(#nameClip-${
                bracketMatch.id
            }-1)">${p1Name || ''}</text>
            `;
        if (bracketMatch.p1.name == null && bracketMatch.p1.id != null)
            bracketMatch.p1.name = null;
        let p2Name: string = bracketMatch.p2.name;
        if (bracketMatch.p2.name == null && bracketMatch.p2.id != null)
            p2Name = bracketMatch.p2.id;

        group.innerHTML += `
            <image x="17" y="18"
            ${
                bracketMatch.p2.avatar
                    ? `href="https://cdn.discordapp.com/avatars/${bracketMatch.p2.id}${bracketMatch.p2.avatar}"`
                    : ''
            }            
            class="img p2" height="16" width="16" clip-path="url(#clipPath-${
                bracketMatch.id
            }-2)" />
            <text x="37" y="33" width="147" height="12" class="pName p2" clip-path="url(#nameClip-${
                bracketMatch.id
            }-2)">${p2Name || ''}</text>
            `;
        if (bracketMatch.p2.name == null && bracketMatch.p2.id != null)
            bracketMatch.p2.name = null;
        if (bracketMatch.p1.score != 0 || bracketMatch.p2.score != 0) {
            let p1Class = '',
                p2Class = '';
            if (bracketMatch.status == 'complete') {
                p1Class =
                    bracketMatch.p1.score > bracketMatch.p2.score
                        ? 'winner'
                        : 'loser';
                p2Class =
                    bracketMatch.p1.score < bracketMatch.p2.score
    ...
    static generateLabels(bracketData: bracketMatch[]) {
        ...
        for (const match of matchElements) {
            ...
            if (curMatch.p1_prereq_identifier) {
                let prereqMatch = document.getElementById(
                    curMatch.p1_prereq_identifier.toString()
                );
                let p1Loser =
                    prereqMatch.querySelector(`.matchLabel`)?.innerHTML;
                match.querySelector('.loserPlaceHolder.top').innerHTML = '';
                if (
                    match.querySelector(`.pName.p1`)?.innerHTML == '' &&
                    !prereqMatch.classList.contains('hidden')
                )
                    match.querySelector(
                        '.loserPlaceHolder.top'
                    ).innerHTML = `Loser of ${p1Loser}`;
            }
            if (curMatch.p2_prereq_identifier) {
                let prereqMatch = document.getElementById(
                    curMatch.p2_prereq_identifier.toString()
                );
                let p2Loser =
                    prereqMatch.querySelector(`.matchLabel`)?.innerHTML;
                match.querySelector('.loserPlaceHolder.bottom').innerHTML = '';
                if (
                    match.querySelector(`.pName.p2`)?.innerHTML == '' &&
                    !prereqMatch.classList.contains('hidden')
                )
                    match.querySelector(
                        '.loserPlaceHolder.bottom'
                    ).innerHTML = `Loser of ${p2Loser}`;
            }
            if (
                curMatch.round == maxRound - 1 &&
                match.querySelector('.loserPlaceHolder')
            ) {
                match.querySelector('.loserPlaceHolder').innerHTML = '';
                if (curMatch.p2.id == null)
                    match.querySelector(
                        '.loserPlaceHolder'
                    ).innerHTML = `Winner of Losers Bracket`;
            }
            if (
                curMatch.round == maxRound &&
                match.querySelector('.loserPlaceHolder')
            ) {
                if (curMatch.p2.id == null)
                    match.querySelector(
...
export class updateMatchDialog implements OnInit {
    ...
    onSubmit(status) {
        ...
        this.updateScore(this.scoreForm.value).subscribe(
            (data) => {
    ...
    updateScore(data: any): Observable<any> {
        return this.http.put(
            `/api/tournament/${this.data.tournamentId}/bracket/${this.data.id}`,
            data
        );
    }
    ...
    async scheduleMatch() {
        try {
            await this.http
                .put(
                    `/api/tournament/${this.data.tournamentId}/bracket/schedule/${this.data.id}`,
                    {
                        matchTime: new Date(
                            (<HTMLInputElement>(
    ...
    setBestOf(event: EventTarget) {
        this.bestOf = (<HTMLInputElement>event).value;
    }
    ...
    async updateBestOf() {
        ...
        try {
            await this.http
                .put(
                    `/api/tournament/${this.data.tournamentId}/bracket/setBestOf/${this.data.id}`,
                    { best_of: this.bestOf }
                )
                .toPromise();
            this.notif.showSuccess('', 'Successfully Scheduled Match');
...
export class generateBracketDialog implements OnInit {
    ...
    updatePlayers(event) {
        let option = this.bracketGenForm.value.users.findIndex(
            (x) => x.id == event.option._parent.id
        );
        if (option > -1) {
            this.users.push(
                this.hiddenUsers.find(
                    (x) =>
                        x.discordId ==
                        this.bracketGenForm.value.users[option].userId
                )
            );
            this.hiddenUsers.splice(
                this.hiddenUsers.findIndex(
                    (x) =>
                        x.discordId ==
                        this.bracketGenForm.value.users[option].userId
                )
            ...
            this.bracketGenForm.value.users.push({
                userId: event.option.value,
                id: event.option._parent.id,
            });
        }

        let usrIndex = this.users.findIndex(
            (x) => x.discordId == event.option.value
        );
        if (usrIndex > -1) {
    ...
    updateLastEvent(event: EventTarget) {
        if (event instanceof HTMLInputElement) this.lastVal = event.value;
    }
    ...
    async onSubmit() {
        let players =
            this.bracketGenForm.value.players != null
                ? this.bracketGenForm.value.players.replace(' ', '').split('\n')
                : null;
        let users =
            this.bracketGenForm.value.users.length > 0
                ? this.bracketGenForm.value.users.map((x) => x.userId)
                : null;
        try {
            const bracketGen: any = await this.http
                .post(
                    `/api/tournament/${this.data.tournamentId}/generateBracket`,
                    {
