...
export class EditStaffComponent implements OnInit {
    ...
    constructor(
        ...
        public data: { tournament: any; staff: staff[] },
    ...
    async ngOnInit(): Promise<void> {
        ...
        for (const staff of this.data.staff) {
            this.users.splice(
                this.users.findIndex((x) => x.discordId == staff.discordId),
                1
            );
        }
    }
    ...
    updateRoles(user: staff, role: role) {
        let userIndex = this.data.staff.findIndex(
            (x) => x.discordId == user.discordId
        );
        let roleIndex = this.data.staff[userIndex].roles.findIndex(
            (x) => x.id == role.id
        );
        if (roleIndex > -1) {
    ...
    async update() {
        ...
        try {
            await this.http
                .post(
                    `/api/tournament/${this.data.tournament.tournamentId}/staff`,
                    { users: postData }
                )
                .toPromise();
